...
import { loadGames, deleteGame, isSupabaseConfigured, getSyncStatus, onSyncStatusChange, flushPendingQueue } from '../lib/supabase'
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsub = onSyncStatusChange((gameId, status) => {
      setSyncStatuses(prev => ({ ...prev, [gameId]: status }))
    })
    return unsub
  }, [])
                  ...
                  <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem', transition: 'border-color 0.15s, transform 0.1s', cursor: 'pointer', flexWrap: 'wrap' }}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      {isSupabaseConfigured() && (() => {
                        const status = syncStatuses[game.id] || getSyncStatus(game.id)
                        if (status === 'synced') return <span title="Synced to cloud" style={{ fontSize: 14 }}>☁️</span>
                        if (status === 'pending') return (
                          <span onClick={async e => { e.preventDefault(); e.stopPropagation(); await flushPendingQueue(); fetchGames() }}
                            title="Pending sync — tap to retry"
                            style={{ fontSize: 12, color: 'var(--accent)', cursor: 'pointer', padding: '1px 6px', borderRadius: 4, background: 'rgba(212,168,67,0.15)', border: '1px solid var(--accent)' }}>
                            ↑ Sync
                          </span>
                        )
                        if (status?.startsWith('error')) return (
                          <span onClick={async e => { e.preventDefault(); e.stopPropagation(); await flushPendingQueue(); fetchGames() }}
                            title={status}
                            style={{ fontSize: 12, color: 'var(--red)', cursor: 'pointer', padding: '1px 6px', borderRadius: 4, background: 'rgba(224,82,82,0.15)', border: '1px solid var(--red)' }}>
                            ⚠ Retry
                          </span>
                        )
                        return null
